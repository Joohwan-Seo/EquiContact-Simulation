...
def main(args):
    ...
    for episode_idx in range(num_episodes):
        ...
        for j in range(ep_len):
            ...
            if action_category == "eef_pose_world" or action_category == "relative_fixed":
                action = actions[j]
        ...
        if pad_len > 0:
            ...
            last_action = actions_buffer[-1]
            data_dict['/observations/state'] += [last_state.copy()] * pad_len
            actions_buffer += [last_action.copy()] * pad_len

            poses_dict_buffer["pos"] += [poses_dict_buffer["pos"][-1]] * pad_len
            poses_dict_buffer["rotm"] += [poses_dict_buffer["rotm"][-1]] * pad_len
        ...
        actions_np = np.zeros((max_timesteps + num_queries, action_dim))
        actions_np[:max_timesteps, :] = np.array(actions_buffer)

        # now each action size is [action_dim, num_queries]
        # is_pad size is [1, num_queries]

        # print(len(poses_dict_buffer['pos']))

        for i in range(max_timesteps):
            if action_category == "relative_fixed":
                current_g = np.eye(4)
                current_g[:3, 3] = poses_dict_buffer["pos"][i]
                current_g[:3, :3] = poses_dict_buffer["rotm"][i]

                action_query_stacked = np.zeros((num_queries, action_dim))
                for j in range(num_queries):
                    g_a = np.eye(4)
                    g_a[:3, 3] = actions_np[i + j][:3]
                    g_a[:3, :3] = R.from_rotvec(actions_np[i + j][3:6]).as_matrix()

                    g_rel = np.linalg.inv(current_g) @ g_a

                    action_query_stacked[j, :] = np.zeros(7)
                    action_query_stacked[j, 0:3] = g_rel[:3, 3]
                    action_query_stacked[j, 3:6] = R.from_matrix(g_rel[:3, :3]).as_rotvec()
                    action_query_stacked[j, -1] = actions_np[i + j][-1]

            else:
                action_query_stacked = actions_np[i:i + num_queries, :]

            #action_query_stacked shape is [num_queries, action_dim]

            data_dict['/action'].append(action_query_stacked)

            pad_mask = (np.arange(num_queries) + i >= max_timesteps)
            data_dict['/is_padded'].append(pad_mask)
        ...
        with h5py.File(dataset_path + '.hdf5', 'w', rdcc_nbytes=1024 ** 2 * 2) as root:
            ...
            action = root.create_dataset('action', (max_timesteps, num_queries, action.shape[0]))
            is_pad = root.create_dataset('is_padded', (max_timesteps, num_queries), dtype=bool)
